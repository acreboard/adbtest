...
namespace :app do
  desc 'a rake task to set up the initial admin and configuration for '\
    'abdreservations site'
  task setup: :environment do
    # Welcome message and create admin user
    puts ''
    puts 'Welcome to abdreservations! Before using your application, we need '\
      'to create an'
    ...
    if AppConfig.all.empty?
      ...
      until AppConfig.first
        ...
        puts 'Home Link Location (e.g. https://abdreservations.pages.dev):'
        ...
        puts 'just created, and finish setting up the configurations that '\
          "we couldn't set"
        puts "from here. Thanks for using abdreservations, and don't "\
          'hesitate to submit any'
        puts 'issues or questions on our github page:'
        puts 'https://github.com/acreboard/abdreservations'
      end
    else
