...
const storage = multer.diskStorage({
    destination:  process.env.APPDATA+'/abdbillcounter/uploads',
...
let settingsDB = new Datastore( {
    filename: process.env.APPDATA+"/abdbillcounter/server/databases/settings.db",
    autoload: true
} );
...
app.post( "/post", upload.single('imagename'), function ( req, res ) {
    ...
    if(req.body.remove == 1) {
        const path = process.env.APPDATA+"/abdbillcounter/uploads/"+ req.body.img;
        try {
          fs.unlinkSync(path)
        } catch(err) {
          console.error(err)
